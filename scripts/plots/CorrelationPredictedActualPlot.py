...
data = pd.read_excel(data_path / 'EntropyDiffPredictedActualplot_k10.xlsx')
...
plt.title("Entropy difference (k=10)", fontsize=18, fontweight='bold')
plt.xlabel("Predicted", fontsize=15)
plt.ylabel("Actual", fontsize=15)

# Add the R and p-values to the bottom right of the figure with increased font size
plt.text(0.95*data["Predicted"].max(), 0.50*data["Actual"].max(), 
         f'R = {r_value:.2f}\np = {p_value:.4f}', 
