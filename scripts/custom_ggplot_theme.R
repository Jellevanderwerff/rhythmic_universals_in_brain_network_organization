theme_jelle <- function() {
  ...
  theme <- list(ggplot2::theme(
    ...
    strip.text = ggplot2::element_text(family = font, size = 16,
                                       face = "bold", color = "#222222"),
    strip.placement = "inside")  # end of ggplot2::theme
    ,
                ggplot2::guides(colour = guide_legend(label.position = "left"),  # second item in list
                                fill = guide_legend(label.position = "left")),  # third item in list
