...
quantized_df = ITIs.copy()
quantized_df['resp_iti_q'] = np.nan
quantized_df['stim_ioi_q'] = np.nan
        ...
        for length, length_df in tempo_df.groupby('length'):
            for sequence_id, sequence_df in length_df.groupby('sequence_id'):
                for stim_resp in ('stim_ioi', 'resp_iti'):
                    ...
                    # Save quantized IOIs
                    if stim_resp == 'stim_ioi':
                        quantized_df.loc[quantized_df.sequence_id == sequence_id, 'stim_ioi_q'] = seq_q.iois
                    else:
                        quantized_df.loc[quantized_df.sequence_id == sequence_id, 'resp_iti_q'] = seq_q.iois
...
# Also save quantized IOIs
quantized_df.to_csv(os.path.join('data', 'experiment', 'processed', 'ITIs_quantized.csv'))
