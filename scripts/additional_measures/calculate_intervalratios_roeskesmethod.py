...
for sequence_id in df.sequence_id.unique():
    ...
    isochrony_increase = isochrony_freq_resp - isochrony_freq_stim
    binary_increase = binary_freq_resp - binary_freq_stim
    ternary_increase = ternary_freq_resp - ternary_freq_stim
    simple_increase = isochrony_increase + binary_increase + ternary_increase
    ...
    df_piece = pd.DataFrame({
        ...
        'isochrony_increase': isochrony_increase,
        'binary_increase': binary_increase,
        'ternary_increase': ternary_increase,
        'simple_ratio_increase': simple_increase
    }, index=[0])
    ...
    df.loc[df.sequence_id == sequence_id, 'isochrony_increase'] = isochrony_increase
    df.loc[df.sequence_id == sequence_id, 'binary_increase'] = binary_increase
    df.loc[df.sequence_id == sequence_id, 'ternary_increase'] = ternary_increase
    df.loc[df.sequence_id == sequence_id, 'simple_ratio_increase'] = simple_increase

    # Also add to ITIs_bytrial df the important bits
    df_bytrial.loc[df_bytrial.sequence_id == sequence_id, 'isochrony_increase'] = isochrony_increase
    df_bytrial.loc[df_bytrial.sequence_id == sequence_id, 'binary_increase'] = binary_increase
    df_bytrial.loc[df_bytrial.sequence_id == sequence_id, 'ternary_increase'] = ternary_increase
    df_bytrial.loc[df_bytrial.sequence_id == sequence_id, 'simple_ratio_increase'] = simple_increase
