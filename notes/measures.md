...
## Absolute stimulus-response asynchrony
This measure gives the amount of stimulus-response asynchrony after tempo normalization. Remember that this was not a synchronization experiment, but that participants copied the stimulus.
...
We followed the method from Roeske et al., by dividing the distribution of measured interval ratios into on-ratio and off-ratio values. This was done by equally dividing up the distribution into adjacent bins, where some are considered on-ratio, and some off-ratio (see Methods section from Roeske for details of which ratios are considered on- and off-ratio).

We did this separetely for the stimulus and the response, and seperately calculated the (normalized by the bin width) counts for **isochrony** and **binary and ternary ratios**.
...
- Then, for each trial we counted the number of integer ratios that fell into the isochronous, binary, or ternary bins, separately for each participant and each of the four conditions.
We then normalized their counts as $count = \frac{\textrm{observations}}{\textrm{total observations} \times \textrm{bin width}}$
