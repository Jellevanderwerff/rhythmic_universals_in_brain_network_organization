...
here::i_am(file.path('0 - dataprep.rmd'))
...
config <- read.ini(here('Input', 'config.txt'))
# Change data types
...
data_participants_info_dir <- here('Input', 'data_participants_info')
data_participants_tapping_dir <- here('Input', 'data_participants_tapping')
...
if (length(data_participants_info_filenames) > 1) {
  ...
  participants.info <-
    read.csv(file.path(data_participants_info_dir,
...
for (i in seq_along(data_participants_tapping_filenames)) {
  ...
  if (endsWith(filename, '.csv')) {
    ...
    assign(sprintf("participant.tapping.%d", pp_id),
...
col_names <- c("pp_id", "age", "gender", "L1", "handedness", "musical_experience", "stim_id", "sequence_id", "length", 'n_resp', "stim_tempo_intended",
...
saveRDS(ITIs, here('Dataframes', 'ITIs.rds'))
write.csv(ITIs, here('Dataframes', 'ITIs.csv'))

# gdata::keep(list=c('calculatePDVs', 'config', 'participants.info', 'splitPythonListString'), sure = TRUE)
...
ITIs.bytrial <- ddply(ITIs, .(pp_id, age, gender, L1, handedness, musical_experience, stim_id, sequence_id,
...
saveRDS(ITIs.bytrial, here('Dataframes', 'ITIs_bytrial.rds'))
write.csv(ITIs.bytrial, here('Dataframes', 'ITIs_bytrial.csv'))

gdata::keep(list=c('calculatePDVs', 'config', 'participants.info', 'pp_ids', 'tapping.raw', 'splitPythonListString', 'ITIs', 'ITIs.bytrial'), sure=TRUE)
